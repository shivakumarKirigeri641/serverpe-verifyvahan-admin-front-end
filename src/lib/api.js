...
export const api = {
  ...
  watch: (o) => request(`/watch${qs(o)}`),
  runWatchSweep: () => request('/watch/run', { method: 'POST' }),
  setWatchPricing: (p) => request('/settings/watch-pricing', { method: 'PUT', body: p }),
  premiumSettings: () => request('/settings/premium'),
  setPremiumSettings: (p) => request('/settings/premium', { method: 'PUT', body: p }),
  ...
  grantPremium: (mobile, reg_no) => request('/premium/grant', { method: 'POST', body: { mobile, reg_no } }),
  gameOverview: () => request('/game/overview'),
  ...
  legal: () => request('/legal'),
  saveLegalTexts: (texts) => request('/legal/texts', { method: 'PUT', body: texts }),
  saveClause: (key, id, body) => request(`/legal/${key}/clause/${id}`, { method: 'PUT', body }),
  addClause: (key, body) => request(`/legal/${key}/clause`, { method: 'POST', body }),
  deleteClause: (key, id) => request(`/legal/${key}/clause/${id}`, { method: 'DELETE' }),
  tickets: (o) => request(`/tickets${qs(o)}`),
  setTicketStatus: (id, status) => request(`/tickets/${id}`, { method: 'PUT', body: { status } }),
